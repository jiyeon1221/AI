...
# ── 3x3 그리드 인접 (position_calculator와 무관한 순수 인덱스 매핑) ──
...
def _build_state_context(state: Dict) -> str:
    ...
    return "\n".join([
        ...
        f"Current Position: x={pos['x']:.3f}, y={pos['y']:.3f}  "
        f"[X moved: {state.get('x_moved', False)}, Y confirmed: {state.get('y_confirmed', False)}]",
    ])
...
def generate_workflow(center: str, direction: str, channel: str,
    ...
    state = {
        ...
        "needs_plot_confirm": False, "x_moved": False, "y_confirmed": False,
    ...
    def _emit(sign, step):
        ...
        # 1a-i: motor X move
        dec = {"tool": "motor_x_move_tool", "params": {"x": round(pos["x"], 3)}}
...
def main():
    ...
    for direction in ("horizontal", "vertical"):
        ...
        for _ in range(12):
            ...
            est = {"x": round(random.uniform(70.0, 130.0), 3),
                   "y": round(random.uniform(70.0, 130.0), 3)}
